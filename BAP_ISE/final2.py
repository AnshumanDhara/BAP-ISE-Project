...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Input(id='uid-input', type='number', placeholder='Enter UID'),
        html.Button('Submit', id='uid-submit-button', n_clicks=0),
        html.Div(id='uid-output-container')
    ])
...
@app.callback(
    [Output('output-container', 'children'),
     ...
     Output('uid-output-container', 'children')],
    [Input('class-dropdown', 'value'),
     Input('subject-dropdown', 'value'),
     Input('type-dropdown', 'value'),
     Input('division-dropdown', 'value'),
     Input('filter-checkbox', 'value'),
     Input('uid-submit-button', 'n_clicks'),
     Input('uid-input', 'value')],
)
def update_output(class_selected, subject_selected, type_selected, division_selected, filter_value, n_clicks, uid):
    ...
    ), defaulter_text, pie_fig, line_fig, bar_fig1, bar_fig2, display_uid_attendance(n_clicks, uid)

def display_uid_attendance(n_clicks, uid):
    if n_clicks > 0 and uid:
        # Initialize an empty DataFrame to store attendance records for the UID
        uid_attendance_df = pd.DataFrame()

        # Iterate through all 20 datasets to find rows where the UID is present
        for df_name, df in globals().items():
            if isinstance(df, pd.DataFrame) and 'UID No.' in df.columns:
                uid_rows = df[df['UID No.'] == uid]
                if not uid_rows.empty:
                    # Append attendance records for the UID in the current dataset to uid_attendance_df
                    uid_attendance_df = pd.concat([uid_attendance_df, uid_rows])

        if not uid_attendance_df.empty:
            # Display attendance records for the UID
            return html.Table(
                # Header
                [html.Tr([html.Th(col, style={'background-color': '#f2f2f2', 'padding': '10px', 'text-align': 'center'}) for col in uid_attendance_df.columns])] +

                # Body
                [html.Tr([
                    html.Td(uid_attendance_df.iloc[i][col], style={'padding': '10px', 'text-align': 'center'})
                    for col in uid_attendance_df.columns
                ]) for i in range(len(uid_attendance_df))],

                style={'width': '100%', 'border-collapse': 'collapse', 'border': '1px solid #ddd', 'margin-top': '10px'}
            )
        else:
            return "No attendance records found for the provided UID."
    else:
        return None
